...
(function() {
  ...
  /**
   * Connects to the socket.io socket server.
   */
  rtc.connect = function() {
     ...
     socket.on('client_disconnect', function (data) {
      ...
      socket.on('get_peers', function(data) {
      	var connectionsrtc = JSON.parse(JSON.stringify(data.connections));
      	for(var i = 0 ; i < connectionsrtc.length ; i++) {
      		if(connectionsrtc[i].id == data.you.id) {
      			connectionsrtc.splice(i, 1);
      		}else {
      			rtc.connections.push(connectionsrtc[i]);
      		}
      ...
      socket.on('remove_stream', function(data) {
      	  console.log(data);
	      rtc.peerConnections[data].removeStream(rtc.remoteStream);
	      document.getElementById('userVideo').src = "";
      });
  ...
  if(rtc._me.role == 'presenter') {
    for (var i = 0; i < rtc.connections.length; i++) {
      var socketId = rtc.connections[i].id;
  ...
  rtc.createPeerConnections = function() {
    for (var i = 0; i < rtc.connections.length; i++) {
      rtc.createPeerConnection(rtc.connections[i].id);
    }
  };
  ...
  rtc.createPeerConnection = function(id) {
    ...
    pc.onaddstream = function(event) {
      // TODO: Finalize this API
      document.querySelector('#videoBox .message').style.display="none";
      rtc.attachStream(event.stream, 'userVideo');
      rtc.remoteStream = event.stream;
  ...
  rtc.createStream = function(onSuccess, onFail) {
  ...
  rtc.addStreams = function() {
      for (var connection in rtc.peerConnections) {
        ...
        rtc.peerConnections[connection].addStream(rtc.localStream);
      }
  };
...
(function($) {
	$('#videoBox').on('click', '.startButton', function(e) {
		rtc.createStream(function(stream) {
			rtc.localStream = stream;
			rtc.attachStream(stream, 'userVideo');
			$('#videoBox .controls').html('<a class="btn alert endButton" href="#">Stop Video</a>');
			document.querySelector('#videoBox .message').style.display="none";
			if(sessionStarted) {
				rtc.addStreams();
			}else {
	...
	$('#videoBox').on('click', '.endButton', function (e) {
    	$('#userVideo').src = "";
    	$('#videoBox .controls').html('<a class="btn success startButton" href="#">Start Video</a>');
    	rtc.localStream.stop();
    	socket.emit('remove_stream', rtc._me.id);
	});
})(jQuery);
